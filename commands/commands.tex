...
As already described in Section~\ref{sec:pipcc-exec}, \pipcmd{pipcc}
is the compiler script for compiling PiP programs for C and C++ and
\pipcmd{pipfc} is for Fortran.
...
environment variable {\tt CC} for \pipcmd{pipcc} or {\tt FC} for
\pipcmd{pipfc}.

By default, \pipcmd{pipcc} and \pipcmd{pipfc} compile program to
...
All options and parameters not for \pipcmd{pipcc} and those Linux
...
The \pipcmd{pip-check} program does not guarantee a program to
run as a PiP program, even if it tells so. 

\subsection{\PIPKW{pip-exec}}

The \pipcmd{pip-exec} command is to invoke PiP tasks derived
from one program in the examples so far. However, \pipcmd{pip-exec}
...
\pipcmd{pips} is the command to output the list of currently running
PiP roots and PiP tasks in the similar way of what the Linux's {\tt
  ps} command does. Here is the example, running three (3)
\pipcmd{pip-exec} each of which execute {\tt a}, {\tt b}, or {\tt c}
PiP tasks. 
...
This \pipcmd{pips} command has many options. Refer PiP man page
(\ref{sec:pip-man}) for more details. 

\subsection{\PIPKW{pip-gdb}}\label{sec:pip-gdb}

\pipcmd{pip-gdb} is PiP-aware version of {\tt gdb} (GNU
