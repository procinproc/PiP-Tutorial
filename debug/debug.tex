...
\pipenv{PIP_STOP_ON_START}. Here, \pipcmd{pips} command is invoked
...
\pipcmd{pips} was used instead in this example.}. Note that 
...
a signal specified by the \pipenv{PIP_GDB_SIGNALS} is delivered, then
\pipcmd{pips} command (Section~\ref{sec:pips}) is invoked to show the
status of the other PiP tasks in the same address space.

\subsection{\PIPKW{PIP_GDB_PATH} and \PIPKW{PIP_GDB_COMMAND}}

When \pipenv{PIP_GDB_PATH} is set to the path to \pipcmd{pip-gdb}
a signal specified by the \pipenv{PIP_GDB_SIGNALS} is delivered, then
